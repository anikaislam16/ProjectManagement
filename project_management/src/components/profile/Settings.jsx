...
const Profile_main = () => {
  ...
  const handleFileInputChange = (event) => {
    ...
    if (file) {
      //file selected na hole eikhane asbe na.
      var reader = new FileReader();
      reader.readAsDataURL(file);
      reader.onload = (e) => {
        //when file successfully loaded in the browser, tokhon ei onload event call hoi.
        const dataSizeInBytes = new TextEncoder().encode(
          JSON.stringify(e.target.result)
        ).length;
        if (dataSizeInBytes < 999990) {
          //this is because , body parser size is set 1MB in the index.js in backend. means, 1 api can send or recieve max. 1MB data at a time
  ...
  const handleCancelOrgClick = () => {
    ...
    setOrganization(
      user.organization ? user.organization : "Your Organization"
    );
  };
