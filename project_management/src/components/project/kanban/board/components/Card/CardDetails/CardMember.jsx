...
const CardMember = (props) => {
  ...
  useEffect(() => {
        ...
        if (response.ok) {
          ...
          if (Array.isArray(data.members)) {
            const fetchMemberDetails = async () => {
              ...
              const filteredArray = updatedMembers.filter(
    ...
    const fetchMembersData = async () => {
      const apiUrl = `${process.env.REACT_APP_HOST}/projects/kanban/${projectId}/${props.bid}/${props.cardId}`;
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.card.members && Array.isArray(data.card.members)) {
            ...
            const memberIds = data.card.members.map(
  ...
  const removeMemberfromCard = async (memberId) => {
    ...
    const key = "members";
    const apiUrl = `${process.env.REACT_APP_HOST}/projects/kanban/${projectId}/${props.bid}/${props.cardId}/${key}/${memberId}`;
  ...
  const handleMemberClick = async (memberId, role, memberName) => {
    //ekhane memberId selected list e ase kina ta check kora hoi. jodi thake, tahole baad dibe. r na thakle include krbe.
    console.log("Selected Members:", selectedMembers);
    const isSelected = selectedMembers.includes(memberId);
    if (isSelected) {
      const response =
        await fetch(`${process.env.REACT_APP_HOST}/projects/kanban/${projectId}/${props.bid}/${props.cardId}
            `);
      if (response.ok) {
        const data = await response.json();
        ...
        for (const task of data.card.task) {
          try {
            const response_task = await fetch(
              `${process.env.REACT_APP_HOST}/test/kanban/${task._id}`
            );
            if (!response_task.ok) {
              continue;
            }
            const data_task = await response_task.json();
            const count = data_task.filter(
              (item) => item.creator_id === memberId
            ).length;
            cnt_task += count;
          } catch (error) {
        ...
        if (cnt_task > 0) {
          setReplaceMember(
            (replaceMember = { name: memberName, id: memberId })
          );
  ...
  const handleReplacement = async (deletedMemberId, replacedId) => {
    ...
    if (deletedMemberId === replacedId) {
      try {
        console.log(
          `${process.env.REACT_APP_HOST}/test/kanban/${projectId}/manytest/${props.bid}/${props.cardId}`
        );
        console.log(deletedMemberId, replacedId);
        const response = await fetch(
          `${process.env.REACT_APP_HOST}/test/kanban/${projectId}/manytest/${props.bid}/${props.cardId}`,
  ...
  return (
    <div
      ...
      style={{ cursor: props.member_role === "admin" ? "pointer" : "default" }}
    >
        ...
        {sortedMembers.map((member) => (
          <li
            ...
            className={`list-group-item d-flex justify-content-between align-items-center ${
              selectedMembers.includes(member.member_id) ? "active" : ""
            }`}
          >
            <div style={{ flex: 1 }}>
              {" "}
              {/* Make sure the username and email container takes full width available except for the badge */}
