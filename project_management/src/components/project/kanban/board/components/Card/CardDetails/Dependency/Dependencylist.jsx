...
const DependencyList = (props) => {
  ...
  const handleAddDependency = async () => {
    ...
    if (isValuePresent) {
      ...
      const storedependency = async () => {
        const apiUrl = `${process.env.REACT_APP_HOST}/projects/kanban/${projectId}/${bid}/${cardId}`;
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.log(data);
            const response1 = await fetch(
              `${process.env.REACT_APP_HOST}/projects/kanban/${projectId}/${projectsData[selectedIndex]._id}/${newValue}`,
              {
                method: "PUT",
                headers: {
                  "Content-Type": "application/json",
  ...
  const RemoveDependency = async (index) => {
    const apiUrl = `${process.env.REACT_APP_HOST}/projects/kanban/${projectId}/${bid}/${cardId}/dependencies/${dependencies[index].matchingCard._id}`;
    try {
      const response = await fetch(apiUrl, {
        method: "DELETE",
        headers: {
      ...
      if (response.ok) {
        ...
        console.log(data);
        const response1 = await fetch(
          `${process.env.REACT_APP_HOST}/projects/kanban/${projectId}/${dependencies[index]._id}/${dependencies[index].matchingCard._id}/workflow/${cardId}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
