  ...
  const handledragpermission = async (memberId) => {
    const updatedMembers = members.map((member) => {
      if (member.member_id === memberId) {
        ...
        return {
          ...member,
          drag: member.drag === "enable" ? "disable" : "enable",
        };
      }
      return member;
    });
    ...
    setMembers(updatedMembers);
    const url = `${process.env.REACT_APP_HOST}/projects/kanban/member/member/${projectId}/${memberId}`;
    try {
      ...
      <div className="center-content">
        ...
        <div className="list-group-container" style={{ maxHeight: "450px" }}>
          <ul
            ...
            style={
              role === "admin" ? { maxHeight: "300px" } : { maxHeight: "430px" }
            }
          >
                ...
                {role === "admin" ? (
                  <div className="col-3">
                    <div class="d-flex justify-content-start align-items-center">
                      ...
                      {member.role === "developer" && (
                        ...
                        >
                          {member.drag === "enable" ? (
                            <div>Enable workflow control</div>
                          ) : (
                            <div> Disable workflow control</div>
                          )}
                        </button>
                      )}
                    </div>
                  ...
                  >
                    {member.role === "developer" && (
                      <div>
                        {member.drag === "enable" ? (
                          <div>Workflow control enabled</div>
                        ) : (
                          <div> Workflow control disabled</div>
                        )}
                      </div>
                    )}
                  </div>
