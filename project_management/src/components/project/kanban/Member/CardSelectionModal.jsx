...
const CardSelectionModal = ({ show, handleClose, member_id, member, projectId, onDelete }) => {
    ...
    const removeMember = async (card) => {
        const apiUrl = `http://localhost:3010/projects/kanban/${projectId}/${card.boardId}/${card.id}/members/${member_id}`;
        try {
        ...
        try {
            for (const card of selectedCards) {
                ...
                if (!checkMemberIdInCardMember) {
                    ...
                    const apiUrl = `http://localhost:3010/projects/kanban/${projectId}/${card.boardId}/${card.id}`;
                    try {
                        const response = await fetch(apiUrl, {
                            method: 'PUT',
                            headers: {
                ...
                const response = await fetch(`http://localhost:3010/test/kanban/${projectId}/manytest/${card.boardId}/${card.id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json'
                    },
