...
const CardSelectionModal = ({
  ...
  const getCards = async () => {
    try {
      ...
      const filteredCards = cards
        .filter((card) =>
          card.members.some((member) => member.member_id === member_id)
        )
        .map((card) => ({
          id: card._id,
          name: card.cardName,
  ...
  useEffect(() => {
    if (member) {
      const filteredMembers = member
        .filter((m) => m.role === "developer" && m.member_id !== member_id)
  ...
  const removeMember = async (card) => {
    const apiUrl = `${process.env.REACT_APP_HOST}/projects/kanban/${projectId}/${card.boardId}/${card.id}/members/${member_id}`;
    try {
  ...
  const handleSubmit = async () => {
    try {
      for (const card of selectedCards) {
        const checkMemberIdInCardMember =
          card.members &&
          card.members.length > 0 &&
          card.members.some((member) => member.member_id === selectedMember);

        if (!checkMemberIdInCardMember) {
          const newValue = { member_id: selectedMember, role: "developer" };
          const apiUrl = `${process.env.REACT_APP_HOST}/projects/kanban/${projectId}/${card.boardId}/${card.id}`;
          try {
            const response = await fetch(apiUrl, {
              method: "PUT",
              headers: {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_HOST}/test/kanban/${projectId}/manytest/${card.boardId}/${card.id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
            ...
            body: JSON.stringify({
              deletedMemberId: member_id,
              replacedId: selectedMember,
            }),
          }
        );
  ...
  return (
    <Modal show={show} onHide={handleClose} centered>
      <Modal.Header closeButton>
        <Modal.Title>
          Assign Cards of Deleted Member to other Developers
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
