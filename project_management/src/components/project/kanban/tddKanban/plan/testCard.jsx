...
export default function TestCard(props) {
    ...
    const getTest = async (tasks) => {
      ...
      for (const task of tasks) {
        try {
          ...
          const countStatusNotPassed = remainingdata.filter(
  ...
  const handleTestStatusChange = async (testId, status, task_id) => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_HOST}/test/kanban/${task_id}/status`,
        {
          method: "PUT",
          headers: {
  ...
  const renderTestCases = () => {
    ...
    return allTest.map((test, index) => {
      ...
      if (test.task_Name !== currentTaskName) {
        ...
        const taskStatus =
          allPassed && othersTestStatus[test.task_id] === 0 ? (
            <img src="/completed.png" height="20" width="20" />
          ) : (
            <img src="/running.png" height="20" width="20" />
          );
        taskHeader = (
          <div key={`header-${index}`} className="task-header">
            <button
              ...
              title={`${
                othersTestStatus[test.task_id]
              } Test Cases from other developers, till not finished yet.`}
              data-bs-toggle="tooltip"
            >
              <div className="task-status">{taskStatus}</div>{" "}
              {/* Align taskStatus to the left */}
              {currentTaskName}
            </button>
            {allPassed && (
              <img
            ...
            <div className="dropdown">
              ...
              >
                <li>
                  <button
                    className={`dropdown-item ${
                      testStatus[test._id] === "pass" ? "pass" : ""
                    }`}
                    onClick={() =>
                      handleTestStatusChange(test._id, "pass", test.task_id)
                    }
                  >
                    Pass
                  </button>
                ...
                <li>
                  <button
                    className={`dropdown-item ${
                      testStatus[test._id] === "refactor" ? "refactor" : ""
                    }`}
                    onClick={() =>
                      handleTestStatusChange(test._id, "refactor", test.task_id)
                    }
                  >
                    Refactor
                  </button>
                ...
                <li>
                  <button
                    className={`dropdown-item ${
                      testStatus[test._id] === "failed" ? "failed" : ""
                    }`}
                    onClick={() =>
                      handleTestStatusChange(test._id, "failed", test.task_id)
                    }
                  >
                    Failed
                  </button>
