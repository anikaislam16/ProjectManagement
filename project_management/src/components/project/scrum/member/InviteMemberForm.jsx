...
const InviteMemberForm = ({ members, onInvite }) => {
  ...
  const addUserToQuestion = async (projectId, projectType, userId) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_HOST}/message/add-users`,
        {
          method: "PUT",
          headers: {
      ...
      console.error("Error adding user to questions", error);
      throw error;
    }
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      const isEmailAlreadyExists = members.some(
      ...
      if (isEmailAlreadyExists) {
        setPopupMessage(
          <div>
            <p>
              The user with email{" "}
              <span style={{ color: "red" }}>{values.email}</span> is already in
              this project.
            </p>
          </div>
        );
        ...
        const handleFetchMemberInfo = async () => {
          try {
            ...
            const response = await fetch(
              `${process.env.REACT_APP_HOST}/signup/${em}`,
              {
                method: "PUT",
                headers: {
                  "Content-Type": "application/json",
              ...
              if (data.message === "Member not found") {
                setPopupMessage(
                  <div>
                    <p>
                      The user with email{" "}
                      <span style={{ color: "red" }}>{values.email}</span> is
                      not registered in our System.
                    </p>
                  </div>
                );
                setShowPopup(true);
                ...
                const response = await fetch(
                  `${process.env.REACT_APP_HOST}/projects/scrum/member/member/${projectId}/${data._id}`,
                  {
                    method: "POST",
                    headers: {
                      "Content-Type": "application/json",
                ...
                if (response.ok) {
                  ...
                  console.log(projectId, projectType, data._id);
                  addUserToQuestion(projectId, projectType, data._id);
