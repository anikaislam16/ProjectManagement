...
const CardMember = (props) => {
  ...
  useEffect(() => {
    ...
    const fetchMembersData = async () => {
      const apiUrl = `${process.env.REACT_APP_HOST}/projects/scrum/${projectId}/${props.bid}/${props.cardId}`;
  ...
  const removeMemberfromCard = async (memberId) => {
    ...
    const key = "members";
    const apiUrl = `${process.env.REACT_APP_HOST}/projects/scrum/${projectId}/${props.bid}/${props.cardId}/${key}/${memberId}`;
  ...
  const handleMemberClick = async (memberId, role, memberName) => {
    ...
    if (isSelected) {
      const response =
        await fetch(`${process.env.REACT_APP_HOST}/projects/scrum/${projectId}/${props.bid}/${props.cardId}
                `);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleReplacement = async (deletedMemberId, replacedId) => {
    ...
    if (deletedMemberId === replacedId) {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_HOST}/test/scrum/${projectId}/manytest/${props.bid}/${props.cardId}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <div
      ...
      style={{
        cursor: props.member_role === "Scrum Master" ? "pointer" : "default",
      }}
    >
      ...
      <ul className="list-group">
        {sortedMembers.map((member) => (
          <li
            key={member.id}
            onClick={() =>
              props.member_role === "Scrum Master"
                ? handleMemberClick(
                    member.member_id,
                    member.role,
                    member.username
