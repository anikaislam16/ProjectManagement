...
const DependencyList = (props) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          data.boards = data.boards.filter(
            (board) =>
              board.completed === false && board.boardType !== "backlog"
          );
  ...
  const correctDurationOfDecendentCard = (newValue) => {
    ...
    const fixingDuration = (currentCard, visited) => {
      ...
      for (const workflowId of currentCard.workflow || []) {
        ...
        if (
          new Date(workflowCard.creationDate) <= new Date(currentCard.dueDate)
        ) {
          const dayDifference = Math.round(
            (new Date(currentCard.dueDate) -
              new Date(workflowCard.creationDate)) /
              (24 * 60 * 60 * 1000)
          );
          ...
          if (workflowId === cardId) {
            addStartDate(workflowCard.creationDate, workflowCard.dueDate);
          }
  ...
  const setStartDueDate = async (workflowboardid, workflowcardid, card) => {
    const apiUrl = `${process.env.REACT_APP_HOST}/projects/scrum/${projectId}/${workflowboardid}/${workflowcardid}`;
    ...
    try {
      ...
      if (response.ok) {
        ...
        const response1 = await fetch(
          `${process.env.REACT_APP_HOST}/projects/scrum/${projectId}/${workflowboardid}/${workflowcardid}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleAddDependency = async () => {
    ...
    if (isValuePresent) {
      ...
      const storedependency = async () => {
        const apiUrl = `${process.env.REACT_APP_HOST}/projects/scrum/${projectId}/${bid}/${cardId}`;
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.log(data);
            const response1 = await fetch(
              `${process.env.REACT_APP_HOST}/projects/scrum/${projectId}/${projectsData[selectedIndex]._id}/${newValue}`,
              {
                method: "PUT",
                headers: {
                  "Content-Type": "application/json",
  ...
  const RemoveDependency = async (index) => {
    const apiUrl = `${process.env.REACT_APP_HOST}/projects/scrum/${projectId}/${bid}/${cardId}/dependencies/${dependencies[index].matchingCard._id}`;
    try {
      const response = await fetch(apiUrl, {
        method: "DELETE",
        headers: {
      ...
      if (response.ok) {
        ...
        console.log(data);
        const response1 = await fetch(
          `${process.env.REACT_APP_HOST}/projects/scrum/${projectId}/${dependencies[index]._id}/${dependencies[index].matchingCard._id}/workflow/${cardId}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
