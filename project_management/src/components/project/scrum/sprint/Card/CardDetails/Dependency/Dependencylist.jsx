...
const DependencyList = (props) => {
    ...
    const handleAddDependency = async () => {
        ...
        if (isValuePresent) {
            ...
            const storedependency = async () => {
                const apiUrl = `http://localhost:3010/projects/scrum/${projectId}/${bid}/${cardId}`;
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        console.log(data);
                        const response1 = await fetch(
                            `http://localhost:3010/projects/scrum/${projectId}/${projectsData[selectedIndex]._id}/${newValue}`,
                            {
                                method: "PUT",
                                headers: {
                                    "Content-Type": "application/json",
    ...
    const RemoveDependency = async (index) => {
        const apiUrl = `http://localhost:3010/projects/scrum/${projectId}/${bid}/${cardId}/dependencies/${dependencies[index].matchingCard._id}`;
        try {
            const response = await fetch(apiUrl, {
                method: "DELETE",
                headers: {
            ...
            if (response.ok) {
                ...
                console.log(data);
                const response1 = await fetch(
                    `http://localhost:3010/projects/scrum/${projectId}/${dependencies[index]._id}/${dependencies[index].matchingCard._id}/workflow/${cardId}`,
                    {
                        method: "DELETE",
                        headers: {
                            "Content-Type": "application/json",
    ...
    return (
        <div>
            ...
            <div className="d-flex justify-content-end">
                {props.role === 'Scrum Master' ? <Button variant="primary" onClick={() => {
