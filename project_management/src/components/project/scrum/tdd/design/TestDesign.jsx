...
const TestDesign = () => {
  ...
  const handleMoveToRequirement = async (cardId) => {
    try {
      ...
      const response = await fetch(`http://localhost:3010/projects/scrum/${projectId}/${boardId}/${cardId}`, {
        method: 'PUT',
        headers: {
          ...
          <Card key={card.cardName} className="card-item">
            <Card.Body>
              ...
              <div className="d-flex justify-content-end" style={{ marginTop: '10px' }}> {/* Align button to the right and add top margin */}
                <Button variant="primary" style={{ marginRight: '10px', marginBottom: '5px' }} onClick={() => handleMoveToRequirement(card._id)}>Move to Requirements</Button> {/* Add margin to the right of the button */}
